...
<form on:submit|preventDefault={submitHandler} class="w-1/2 my-[10px] mx-auto py-3 px-5 bg-[#f4f7f8] rounded-lg text-center">
    ...
    <input type="text" bind:value={role.name} disabled={!isEditing} required class="text-base h-auto mb-8 outline-0 p-[15px] w-[100%] bg-[#e8eeef] border-2 focus:border-[#0078c8] disabled:text-[#8a97a9]"/>
    </fieldset>
    {#if isEditing}
        <button type="submit" class="w-[100%] border-2 border-[#0078c8] p-2 mb-5 hover:bg-blue-400">Zakończ edycję</button>
    {:else}
